...
def netG(z, y, BATCH_SIZE):
   ...
   conv5 = tcl.convolution2d_transpose(conv4, 3, 5, 2, activation_fn=tf.nn.tanh, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_conv5')
...
def netD(input_images, y, BATCH_SIZE, LOSS, reuse=False):
   ...
   with tf.variable_scope(sc, reuse=reuse):
      ...
      conv5 = tcl.conv2d(conv4, 512, 5, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='d_conv5')
      if LOSS != 'wgan': conv5 = tcl.batch_norm(conv5)
      conv5 = lrelu(conv5)

      conv6 = tcl.conv2d(conv5, 1, 4, 1, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='d_conv6')
